...
#define DB_NSData @"DB_NSData"
#define DB_UIImage @"DB_UIImage"
    ...
    while ([set next]) {
        ...
        for (int i=0; i<self.columeNames.count; i++) {
            ...
            else if([columeType isEqualToString:@"UIImage"])
            {
                ...
                if([SandboxFile IsFileExists:[SandboxFile GetPathForDocuments:filename inDir:DB_UIImage]])
                {
                    UIImage* img = [UIImage imageWithContentsOfFile:[SandboxFile GetPathForDocuments:filename inDir:DB_UIImage]];
            ...
            {
                NSString* filename = [set stringForColumn:columeName];
                if([SandboxFile IsFileExists:[SandboxFile GetPathForDocuments:filename inDir:DB_NSData]])
                {
                    NSData* data = [NSData dataWithContentsOfFile:[SandboxFile GetPathForDocuments:filename inDir:DB_NSData]];
    ...
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         ...
         for (int i=0; i<self.columeNames.count; i++) {
             ...
             {
                 NSString* filename =[NSString stringWithFormat:@"%@",model.fileName];
                 [UIImageJPEGRepresentation(value, 1) writeToFile:[SandboxFile GetPathForDocuments:filename inDir:DB_UIImage] atomically:YES];
                 value = filename;
             }
             else if([value isKindOfClass:[NSData class]])
             {
                 NSString* filename = [NSString stringWithFormat:@"%@",model.fileName];
                 [value writeToFile:[SandboxFile GetPathForDocuments:filename inDir:DB_NSData] atomically:YES];
                 value = filename;
             }
    ...
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         ...
         /**Author:Ronaldo Description:删除data文件*/
         ...
         for (int i=0; i<self.columeNames.count; i++) {
             ...
             if([value isKindOfClass:[UIImage class]])
             {
                filePath = [SandboxFile GetPathForDocuments:filename inDir:DB_UIImage];
             }
             else if([value isKindOfClass:[NSData class]])
             {
                    filePath = [SandboxFile GetPathForDocuments:filename inDir:DB_NSData];
             }
             [SandboxFile DeleteFile:filePath];
         }
