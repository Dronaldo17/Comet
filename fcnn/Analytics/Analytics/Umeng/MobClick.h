 ...
 *介绍:
 *		类方法设置是否让umeng SDK 捕捉和记录你的crash log
 *参数说明:
 *		(BOOL)value:默认值为YES
 *	    设置为NO,umeng SDK 不再捕捉和记录你的crash log,需要在setDelegate方法之前调用
 */
 ...
 *介绍:
 *		类方法，检查当前app是否有更新，有更新弹出UIAlertView提示用户,当用户点击升级按钮时app会跳转到你预先设置的网址。
 *		无更新不做任何操作。
 *	    你需要先在服务器端设置app版本信息，默认渠道是App Store.
 ...
 *      如果你想自己控制自动更新操作流程，请实现MobClickDelegate的appUpdate方法。
 *
 *返回值:
 *      无
 *
 ...
 *
 *参数说明:
 *		 当你调用[MobClick checkUpdate]方法后，系统会自动调用此方法appUpdate，
